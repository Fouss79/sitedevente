...
import Etoiles from "./Etoiles"; // si tu as un composant pour ça
...
export default function ProduitDetail() {
  ...
  useEffect(() => {
    const fetchProduit = async () => {
      try {
        ...
        const imagesRes = await axios.get(`http://localhost:8080/api/produits/${res.data.id}/images`);
        setImages(imagesRes.data);
        const cat = await axios.get(`http://localhost:8080/api/produits/categorie/${res.data.categorieId}`);
        setProdcat(cat.data);
      } catch (err) {
  ...
  return (
    <div className="max-w-7xl  px-8 ">
      <h1 className='text-3xl font-bold text-2xl mb-4 text-[#036c94]'>Details du produit</h1>
 ...
 {produit.moyenne !== undefined && (
  ...
  ...
          ...
          <div className="flex items-center mb-10 gap-3">
            <Image
              src={`http://localhost:8080/${produit.vendeurImage}`}
              alt="vendeur"
              width={50}
              height={50}
  ...
  <h2 className="text-2xl text-white font-bold bg-[#036c94] px-4 py-2 rounded-t">PRODUITS SIMILAIRES</h2>
